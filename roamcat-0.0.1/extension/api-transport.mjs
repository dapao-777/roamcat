/**
 * @file extension/api-transport.mjs
 * 文件职责：自备API传输层——结构化输出能力探测、模型列表拉取与分服务商并发调度。
 * 主要内容：listProviderModels/performProviderRequest；json_schema探测失败降级json_object，探测缓存1小时；
 *   Jev判定在typesafe.ai域名走/systemone原生协议，其余地址走Requesty chat/completions转发。
 * 模块边界：领域层；只依赖api-providers与gloss校验；不存密钥。
 *
...
// 探测结果随 SW 空闲回收丢失；持久化到 session 存储可跨 SW 生命周期复用（键是
// provider/endpoint/model/key 组合的 SHA-256，不落地明文密钥）。本模块属领域层，
// Node 侧 import 时不得触碰 chrome，全部经 capabilityStorage() 惰性探测。
const OUTPUT_CAPABILITY_KEY='apiOutputCapabilities';
let outputCapabilitiesReady=null;
function capabilityStorage(){try{return globalThis.chrome?.storage?.session||null;}catch{return null;}}
function loadOutputCapabilities(){
  if(outputCapabilitiesReady)return outputCapabilitiesReady;
  const store=capabilityStorage();
  outputCapabilitiesReady=(store?store.get(OUTPUT_CAPABILITY_KEY).then(data=>{
    const saved=data?.[OUTPUT_CAPABILITY_KEY];
    if(saved&&typeof saved==='object'&&!Array.isArray(saved))for(const [key,entry]of Object.entries(saved))if(entry&&(entry.mode==='json_schema'||entry.mode==='json_object')&&Number.isFinite(entry.at))outputCapabilities.set(key,{mode:entry.mode,at:entry.at});
  }).catch(()=>{}):Promise.resolve());
  return outputCapabilitiesReady;
}
function persistOutputCapabilities(){
  const store=capabilityStorage();if(!store)return;
  store.set({[OUTPUT_CAPABILITY_KEY]:Object.fromEntries(outputCapabilities)}).catch(()=>{});
}
...
async function rejectsSchemaFormat(response){
  ...
  const namesFormat=/json_schema|response_format|text\.format|结构化/i.test(message)||['response_format','response_format.type','text.format','text.format.type'].includes(param);
  return namesFormat&&(/not supported|unsupported|does not support|not available|unavailable|only supports?|must be (?:one of|json_object)|supported (?:values|types)|not one of|不支持|不兼容|无效/i.test(message)||error.code==='unsupported_parameter'||error.code==='unsupported_value');
}

async function probeOutputMode(service,protocol,signal){
  aborted(signal);
  ...
  const format=schemaFormat(schema,'roamcat_capability');
  ...
  const value=await jsonResponse(response);responseError(value);let text;
  if(protocol==='chat'){
...
async function outputMode(service,protocol,signal){
  aborted(signal);
  // Step Plan documents JSON Mode as json_object; probing json_schema on /step_plan often 400s.
  if(service.providerId==='stepfun'||stepfunHost(service.baseUrl))return 'json_object';
  ...
  if(!entry||entry.controller.signal.aborted){
    ...
    entry.promise=probeOutputMode(service,protocol,entry.controller.signal).then(mode=>{
      if(!entry.controller.signal.aborted){outputCapabilities.delete(key);outputCapabilities.set(key,{mode,at:Date.now()});while(outputCapabilities.size>64)outputCapabilities.delete(outputCapabilities.keys().next().value);persistOutputCapabilities();}
      return mode;
...
function endpointFor(service,kind,stream=false){const base=serviceBase(service),model=encodeURIComponent(service.model||'');const path=cleanPath(base.pathname);
  ...
  if(kind==='chat'){
    if(stepfunHost(base.href)&&/\/step_plan$/i.test(path)){const url=new URL(base.href);url.pathname=path+'/v1/chat/completions';return url;}
    return /\/chat\/completions$/i.test(path)?base:appendPath(base,'chat/completions');
  }
...
async function checkedFetch(url,init,service,protocol,detectSchemaSupport=false){const base=serviceBase(service);assertSameOrigin(base,url);let response;try{response=await fetch(url,{...init,redirect:'error',credentials:'omit',headers:{...authHeaders(service,protocol),...init.headers}});}catch(error){if(error?.name==='AbortError')throw error;throw transportError('无法连接服务，请检查网络、API 地址与服务跨域支持。','NETWORK');}
  ...
  const message=[401,403].includes(response.status)?'服务拒绝访问，请检查 API Key、模型权限与账户状态。':response.status===429?'服务额度不足或请求过快，请稍后重试或检查账户。':method==='GET'?`无法获取模型列表（HTTP ${response.status}）。该通道可能没有 /models 目录接口。`:incompatible?'服务或模型不兼容无思考模式、结构化输出或当前原生协议，已停止且不会删除参数重试。':`服务返回 HTTP ${response.status}，请检查 API 地址和模型。`;
  throw transportError(message,[401,403].includes(response.status)?'AUTH':response.status===429?'RATE_LIMIT':method==='GET'?'HTTP':incompatible?'INCOMPATIBLE_REQUEST':'HTTP',{httpStatus:response.status});
}
...
function stepfunHost(value){
  try{const host=new URL(value||'https://invalid.invalid').hostname.toLowerCase();return host==='api.stepfun.com'||host==='api.stepfun.ai'||host.endsWith('.stepfun.com')||host.endsWith('.stepfun.ai');}
  catch{return /stepfun\.(?:com|ai)/i.test(value||'');}
}
function typesafeHost(value){
  try{const host=new URL(value||'https://invalid.invalid').hostname.toLowerCase();return host==='typesafe.ai'||host.endsWith('.typesafe.ai');}
  catch{return /(?:^|[./])typesafe\.ai(?:$|[/:])/i.test(value||'');}
}
// StepFun 始终带 reasoning_effort 做推理，思考加输出经常超过常规 API 超时；
// Step Plan 订阅通道排队时更慢。这里只放宽阶跃星辰，其他服务商保持 25 秒快速失败。
export function providerRequestTimeoutMs(service){
  try{
    if(service?.providerId==='stepfun'||stepfunHost(service?.baseUrl))return 60000;
  }catch{/* fall through to default */}
  return 25000;
}
function reasoningEffortOption(service,fallback=''){
  const value=service?.options?.reasoningEffort;
...
function chatThinking(service){const id=service.providerId,model=service.model||'',base=service.baseUrl||'';
  ...
  if(id==='stepfun'||stepfunHost(base)){
    const effort=reasoningEffortOption(service,'low');
    return {reasoning_effort:effort==='none'?'low':effort};
  }
  if(id==='openai-compatible'){
    const effort=reasoningEffortOption(service);
    if(effort==='low'||effort==='medium'||effort==='high')return {reasoning_effort:effort};
    const host=new URL(base).hostname;if((host==='api.openai.com'||host.endsWith('.openai.com'))&&/^(?:chatgpt-4o|gpt-(?:3\.5|4o?))(?:[-_.]|$)/i.test(model))return {};return /deepseek/i.test(model)||/deepseek/i.test(base)?{thinking:{type:'disabled'}}:{reasoning_effort:effort||'none'};
  }
...
function chatMessageText(choice){
  ...
  if(Array.isArray(delta?.content))return textParts(delta.content);
  if(typeof message?.content==='string')return message.content;
  if(Array.isArray(message?.content))return textParts(message.content);
  return '';
}
function chatMaxTokens(payload,service){
  const chars=JSON.stringify(payload||'').length;
  const reasoning=service.providerId==='stepfun'||stepfunHost(service.baseUrl);
  if(reasoning||chars>4000)return 16384;
  return 8192;
}
function parseChatOutput(choice){
  const reason=choice?.finish_reason,text=chatMessageText(choice);
  if(reason==='length'){
    if(String(text).trim()){
      try{return parseObject(text);}
      catch{throw transportError('服务输出达到长度上限。','OUTPUT_LIMIT');}
    }
    throw transportError('服务输出达到长度上限。思考过程占用了输出额度，请降低思考等级或缩短段落。','OUTPUT_LIMIT');
  }
  if(reason!=null&&reason!=='stop'&&reason!=='end_turn'&&!String(text).trim())throw transportError('服务生成异常终止。','PROVIDER_ERROR');
  return parseObject(text);
}
async function performChat(service,payload,instructions,schema,options){const url=endpointFor(service,'chat');const body={model:service.model,temperature:0.2,max_tokens:chatMaxTokens(payload,service),response_format:options.outputMode==='json_schema'?{type:'json_schema',json_schema:{name:'result',strict:true,schema}}:{type:'json_object'},...chatThinking(service),...(options.onContent?{stream:true}:{}),messages:[{role:'system',content:systemPrompt(instructions,schema)},{role:'user',content:JSON.stringify(payload)}]};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'chat');if(streamingResponse(response))return parseObject(await readSSE(response,(_event,value)=>{const choice=value.choices?.[0],reason=choice?.finish_reason;if(reason==='length')return String(chatMessageText(choice)||'').trim()?{delta:chatMessageText(choice),done:true}:{error:true};if(reason!=null&&reason!=='stop'&&reason!=='end_turn')return {error:true};return {delta:chatMessageText(choice),done:reason==='stop'||reason==='end_turn'};},options));const value=await jsonResponse(response);responseError(value);return parseChatOutput(value.choices?.[0]);}
async function performResponses(service,payload,instructions,schema,options){const url=endpointFor(service,'responses');const body={model:service.model,instructions:systemPrompt(instructions,schema),input:JSON.stringify(payload),temperature:0.2,max_output_tokens:8192,text:{format:options.outputMode==='json_schema'?schemaFormat(schema):{type:'json_object'}},...responsesThinking(service),...(options.onContent?{stream:true}:{})};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'responses');if(streamingResponse(response)){const text=await readSSE(response,(event,value)=>{if(event==='response.output_text.delta'||value.type==='response.output_text.delta')return {delta:value.delta||''};if(event==='response.completed'||value.type==='response.completed')return {done:value.response?.status!=='incomplete'};if(event==='response.incomplete'||value.type==='response.incomplete'||event==='response.failed'||value.type==='response.failed')return {error:true};return {};},options);return parseObject(text);}const value=await jsonResponse(response);responseError(value);if(value.status==='incomplete')throw transportError(value.incomplete_details?.reason==='max_output_tokens'?'服务输出达到长度上限。':'服务生成异常终止。','PROVIDER_ERROR');if(value.status&&value.status!=='completed')throw transportError('服务生成异常终止。','PROVIDER_ERROR');const text=typeof value.output_text==='string'?value.output_text:(value.output||[]).flatMap(item=>item?.content||[]).filter(item=>item?.type==='output_text').map(item=>item.text||'').join('');return parseObject(text);}
async function performAnthropic(service,payload,instructions,schema,options){const url=endpointFor(service,'anthropic');const body={model:service.model,max_tokens:8192,temperature:0.2,...(/^claude-(?:haiku|sonnet|opus)-4/i.test(service.model||'')?{thinking:{type:'disabled'}}:{}),system:systemPrompt(instructions,schema),messages:[{role:'user',content:JSON.stringify(payload)}],...(options.onContent?{stream:true}:{})};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'anthropic');if(streamingResponse(response)){return parseObject(await readSSE(response,(event,value)=>{if(event==='content_block_delta'&&value.delta?.type==='text_delta')return {delta:value.delta.text||''};if(event==='message_delta'){const reason=value.delta?.stop_reason;if(reason==='max_tokens')return {error:true};if(reason&&reason!=='end_turn'&&reason!=='stop_sequence')return {error:true};return {};}if(event==='message_stop')return {done:true};if(event==='error')return {error:true};return {};},options));}const value=await jsonResponse(response);responseError(value);if(value.stop_reason==='max_tokens')throw transportError('服务输出达到长度上限。','OUTPUT_LIMIT');if(value.stop_reason&&value.stop_reason!=='end_turn'&&value.stop_reason!=='stop_sequence')throw transportError('服务生成异常终止。','PROVIDER_ERROR');return parseObject(textParts(value.content));}
async function performGoogle(service,payload,instructions,schema,options){const url=endpointFor(service,'google',Boolean(options.onContent));if(options.onContent)url.searchParams.set('alt','sse');const body={systemInstruction:{parts:[{text:systemPrompt(instructions,schema)}]},contents:[{role:'user',parts:[{text:JSON.stringify(payload)}]}],generationConfig:{temperature:0.2,maxOutputTokens:8192,responseMimeType:'application/json',responseJsonSchema:schema,...googleThinking(service)}};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'google');if(streamingResponse(response)){return parseObject(await readSSE(response,(_event,value)=>{const candidate=value.candidates?.[0],reason=candidate?.finishReason;if(reason&&reason!=='STOP')return {error:reason==='MAX_TOKENS'?'服务输出达到长度上限。':'服务生成异常终止。'};return {delta:textParts(candidate?.content?.parts),done:reason==='STOP'};},options));}const value=await jsonResponse(response);responseError(value);const candidate=value.candidates?.[0];if(candidate?.finishReason&&candidate.finishReason!=='STOP')throw transportError(candidate.finishReason==='MAX_TOKENS'?'服务输出达到长度上限。':'服务生成异常终止。','PROVIDER_ERROR');return parseObject(textParts(candidate?.content?.parts));}
async function performBedrock(service,payload,instructions,schema,options){const url=endpointFor(service,'bedrock');const body={system:[{text:systemPrompt(instructions,schema)}],messages:[{role:'user',content:[{text:JSON.stringify(payload)}]}],inferenceConfig:{maxTokens:8192,temperature:0.2},...(/anthropic\.claude-(?:haiku|sonnet|opus)-4/i.test(service.model||'')?{additionalModelRequestFields:{thinking:{type:'disabled'}}}:{})};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'bedrock');const value=await jsonResponse(response);responseError(value);if(value.stopReason&&value.stopReason!=='end_turn'&&value.stopReason!=='stop_sequence')throw transportError(value.stopReason==='max_tokens'?'服务输出达到长度上限。':'服务生成异常终止。','PROVIDER_ERROR');const text=textParts(value.output?.message?.content);if(options.onContent)await options.onContent(text);return parseObject(text);}
async function performCohere(service,payload,instructions,schema,options){const url=endpointFor(service,'cohere');const body={model:service.model,temperature:0.2,max_tokens:8192,...(/reasoning/i.test(service.model||'')?{thinking:{type:'disabled'}}:{}),response_format:{type:'json_object',schema},messages:[{role:'system',content:systemPrompt(instructions,schema)},{role:'user',content:JSON.stringify(payload)}],...(options.onContent?{stream:true}:{})};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'cohere');if(streamingResponse(response))return parseObject(await readSSE(response,(_event,value)=>{if(value.type==='content-delta')return {delta:value.delta?.message?.content?.text||''};if(value.type==='message-end'){const reason=value.delta?.finish_reason;if(reason==='MAX_TOKENS')return {error:true};return {done:!reason||reason==='COMPLETE'||reason==='STOP_SEQUENCE',error:Boolean(reason&&reason!=='COMPLETE'&&reason!=='STOP_SEQUENCE')};}return {};},options));const value=await jsonResponse(response);responseError(value);if(value.finish_reason==='MAX_TOKENS')throw transportError('服务输出达到长度上限。','OUTPUT_LIMIT');if(value.finish_reason&&value.finish_reason!=='COMPLETE'&&value.finish_reason!=='STOP_SEQUENCE')throw transportError('服务生成异常终止。','PROVIDER_ERROR');return parseObject(typeof value.message?.content?.[0]?.text==='string'?value.message.content[0].text:'');}
async function performOllama(service,payload,instructions,schema,options){const url=endpointFor(service,'ollama');const body={model:service.model,think:false,format:schema,stream:Boolean(options.onContent),options:{temperature:0.2,num_predict:8192},messages:[{role:'system',content:systemPrompt(instructions,schema)},{role:'user',content:JSON.stringify(payload)}]};const response=await checkedFetch(url,{method:'POST',signal:options.signal,body:JSON.stringify(body)},service,'ollama');if(options.onContent)return parseObject(await readNDJSON(response,value=>({delta:value.message?.content||'',done:value.done===true&&value.done_reason!=='length',error:value.done===true&&value.done_reason==='length'}),options));const value=await jsonResponse(response);responseError(value);if(value.done_reason==='length')throw transportError('服务输出达到长度上限。','OUTPUT_LIMIT');return parseObject(value.message?.content);}
...
async function performJev(service,payload,options={}){
  ...
  const native=typesafeHost(service.baseUrl);
  const body=native
    ?{model:String(service.model||'').replace(/^typesafe\//i,''),state,questions}
    :{model:service.model,messages:[{role:'user',content:state}],response_format:{type:'questions',questions},stream:false};
  const url=appendPath(serviceBase(service),native?'systemone':'chat/completions');
  ...
  let parsed;
  if(native)parsed=value?.answers;
  else{
    ...
    try{parsed=JSON.parse(text);}catch{throw transportError('判定服务返回了无效 JSON。','JEV_FORMAT');}
  }
...
function parseStepfunModels(value){
  const rows=Array.isArray(value?.data)?value.data:[];
  const chat=rows.filter(item=>{
    const protocols=item?.supported_protocols;
    if(Array.isArray(protocols)&&protocols.length)return protocols.includes('chat');
    const id=typeof item?.id==='string'?item.id:'';
    return /^step-/.test(id)&&!/(?:tts|asr|audio|image|gui|music|search)/i.test(id);
  });
  return normalizedModels(chat,item=>item.id,item=>item.id);
}
async function listStepfunModels(service,signal){
  const base=serviceBase(service);
  const path=cleanPath(base.pathname);
  const originModels=new URL('/v1/models',base.origin+'/');
  const planModels=new URL('/step_plan/v1/models',base.origin+'/');
  const localModels=appendPath(modelsBase(service),'models');
  const candidates=/\/step_plan(?:\/|$)/i.test(path)?[planModels,originModels,localModels]:[originModels,localModels];
  let lastError;
  for(const url of candidates){
    aborted(signal);
    try{
      const response=await checkedFetch(url,{method:'GET',signal},service,'chat');
      const value=await jsonResponse(response);responseError(value);
      const models=parseStepfunModels(value);
  ...
  if(/\/step_plan(?:\/|$)/i.test(path))return STEPFUN_MODELS.map(model=>({...model}));
  throw lastError||transportError('无法获取阶跃星辰模型列表。','HTTP');
}
export async function listProviderModels(service,{signal}={}){const provider=providerFor(service),protocol=service.providerId==='azure'?(service.options?.apiMode==='chat'?'chat':'responses'):provider.protocol;if(service.providerId==='azure'||protocol==='bedrock'||protocol==='responses'&&service.providerId==='open-responses')unsupportedModels();
  if(service.providerId==='stepfun'||stepfunHost(service.baseUrl))return listStepfunModels(service,signal);
  let url,parse;
